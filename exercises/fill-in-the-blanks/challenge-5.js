...
const challenge5 = (str) =>
  new Promise((res, rej) => {
    ...
    .then((resolvedVal) => {
      resolvedVal.pass =
        resolvedVal.status === 10 && hasVowel(resolvedVal.value);
      return resolvedVal;
    })
    .catch((rejectedVal) => {
      rejectedVal.pass =
        rejectedVal.status === 20 && !hasVowel(rejectedVal.value);
      return rejectedVal;
    });
...
setTimeout(() => {
  ...
  const challenge5Tests = Array.from(new Array(50), () =>
    challenge5(randomString())
  );
